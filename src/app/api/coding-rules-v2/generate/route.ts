...
export async function POST(request: NextRequest) {
  try {
    ...
    // 检查重复
    let dupSql = `SELECT * FROM generated_codes_v2 WHERE rule_id = '${ruleId}' AND second_category_id = '${secondCategoryId}' AND material_name = '${materialName}'`;
    const existing = await db.execute(dupSql);
    ...
    // 获取编码
    const rule = (await db.execute(`SELECT * FROM coding_rules_v2 WHERE rule_id = '${ruleId}'`)).rows[0];
    const second = (await db.execute(`SELECT * FROM coding_categories_v2 WHERE category_id = '${secondCategoryId}'`)).rows[0];
    ...
    if (thirdCategoryId && thirdCategoryId !== '0' && thirdCategoryId !== '') {
      const t = (await db.execute(`SELECT * FROM coding_categories_v2 WHERE category_id = '${thirdCategoryId}'`)).rows[0];
      thirdCode = String(t.code);
      thirdName = t.name;
      seqCatId = thirdCategoryId;
      
      if (processCategoryId && processCategoryId !== '0' && processCategoryId !== '') {
        const p = (await db.execute(`SELECT * FROM coding_categories_v2 WHERE category_id = '${processCategoryId}'`)).rows[0];
    ...
    const seq = (await db.execute(`SELECT * FROM coding_sequences_v2 WHERE category_id = '${seqCatId}'`)).rows[0];
    if (seq) {
      seqNum = Number(seq.sequence_number) + 1;
      seqId = seq.sequence_id;
      await db.execute(`UPDATE coding_sequences_v2 SET sequence_number = sequence_number + 1, updated_at = CURRENT_TIMESTAMP WHERE sequence_id = '${seqId}'`);
    } else {
      const newSeq = await db.execute(`INSERT INTO coding_sequences_v2 (category_id, sequence_number) VALUES ('${seqCatId}', 1) RETURNING sequence_id`);
    ...
    // 保存
    const thirdVal = thirdCatId && thirdCatId !== '0' && thirdCatId !== '' ? `'${thirdCatId}'` : 'NULL';
    const processVal = processCatId && processCatId !== '0' && processCatId !== '' ? `'${processCatId}'` : 'NULL';
    await db.execute(`INSERT INTO generated_codes_v2 (code, material_name, rule_id, second_category_id, third_category_id, process_category_id, sequence_number, version, project_name, created_at) VALUES ('${code}', '${materialName}', '${ruleId}', '${secondCategoryId}', ${thirdVal}, ${processVal}, ${seqNum}, '${version}', '${projectName}', CURRENT_TIMESTAMP)`);
